...
class adminProductController {
    ...
    try {
      ...
      if (getCategory.id === parseInt(categoryId)) {
        if (getSubCategoryId.id === parseInt(subCategoryId)) {
          ...
          return successResponse("Product updated sucessfully", final_update);
        }
      }
    } catch (error) {
      console.log("---------->", error);
  ...
  async deleteProduct(req, res) {
    try {
      const deleteData = Product.destroy({
        where: {
          id: req.params.id,
        },
      });
      return successResponse("Product deleted sucessfully", deleteData);
    } catch (error) {
      return internalServerErrorResponse(error.message);
    }
  }
